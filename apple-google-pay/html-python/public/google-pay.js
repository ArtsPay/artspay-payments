...
function GoogleStartPayment() {
  ...
  // Each stage passes null forward once something's already failed, so a
  // config error short-circuits before Google's payment sheet ever opens
  // instead of surfacing as a confusing error inside Google's own UI.
  fetch('/api/google-pay/config')
    .then(function (res) { return res.json().then(function (data) { return { ok: res.ok, data: data }; }); })
    .then(function (config) {
      if (!config.ok) {
        showResult(false, config.data);
        return null;
      }
      var paymentDataRequest = {
        ...
        allowedPaymentMethods: [Object.assign({}, GoogleCardPaymentMethod, {
          ...
          tokenizationSpecification: {
            ...
            parameters: { gateway: 'fatzebra', gatewayMerchantId: config.data.gatewayMerchantId },
          },
        })],
    ...
    .then(function (result) {
      if (!result) return;
      showResult(result.ok && result.data.successful, result.data);
    });
}
